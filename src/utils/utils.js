...
export async function apiReq(
  ...
  requestOptions = {},
   ...
   console.log(endPoint,headers,data, 'endPoint-->');
  ...
  return new Promise(async (res, rej) => {
    ...
    axios[method](endPoint, data, {headers})
      .then((result) => {
        const {data} = result;
         console.log(endPoint,data,'endPointResult-->');
      ...
      .catch((error) => {
         console.log(endPoint,error?.response, 'error>error');
        if (error && error?.response && error?.response.status === 401) {
          sessionHandler(error.response.data.message+endPoint);
          return rej(error);
        }
        if (error && error.response && error.response.data) {
          if (!error.response.data.error) {
            return rej({
              ...error.response.data,
              error: error.response.data.error+endPoint || 'Network Error'+endPoint,
            });
          }
          return rej(error.response.data);
        } else {
          return rej({error: 'Network Error', message: 'Network Error'+endPoint});
        }
        return rej(error);
      });
  });
